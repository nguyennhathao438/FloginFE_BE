...
describe("Log Account Unit Test", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  test("TC01: nên khóa tài khoản sau 5 lần đăng nhập sai", async () => {
  ...
  test("TC02: nên reset số lần thất bại khi đăng nhập thành công", async () => {
  ...
  test("TC03: nên khôi phục trạng thái khóa từ localStorage khi component mount", async () => {
  ...
  test("TC04: nên ngăn chặn đăng nhập khi tài khoản bị khóa", async () => {
    // Giả lập tài khoản đang bị khóa
    const lockTime = Date.now() + 300000;
    localStorageMock.setItem("loginLockUntil", lockTime.toString());
    localStorageMock.setItem("failedLoginAttempts", "5");
  ...
  test("TC05: nên hiển thị lỗi validation khi nhập liệu không hợp lệ", async () => {
    renderLogin();
  ...
  test("TC06: nên hiển thị số lần thử còn lại sau mỗi lần đăng nhập sai", async () => {
    mockLoginService.mockRejectedValue(new Error("Invalid credentials"));
  ...
  test("TC07: nên hiển thị thông báo khóa khi cố đăng nhập trong trạng thái khóa", async () => {
    // Giả lập tài khoản đang bị khóa
    const lockTime = Date.now() + 300000;
    localStorageMock.setItem("loginLockUntil", lockTime.toString());
    localStorageMock.setItem("failedLoginAttempts", "5");
  ...
  test("TC08: nên tự động mở khóa sau khi hết thời gian", async () => {
    // Sử dụng fake timers
    jest.useFakeTimers();
  ...
  test("TC09: không nên khóa tài khoản khi đăng nhập thành công trước 5 lần thất bại", async () => {
    // Giả lập 3 lần thất bại trước đó
    localStorageMock.setItem("failedLoginAttempts", "3");
